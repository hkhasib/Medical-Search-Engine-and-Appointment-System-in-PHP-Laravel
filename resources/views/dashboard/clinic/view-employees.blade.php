    ...
    <div class="page-wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-12">
                    <div class="white-box">
                        <h3 class="box-title m-b-0">All Employee List</h3>
                        @if(session('success'))
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group m-t-10">
                                <select class="form-control" name="clinic_id" required>
                                    <option selected value disabled>--Select Clinic--</option>
                                    @foreach(session('clinics') as $clinic)
                                        <option value="{{$clinic->id}}">{{$clinic->name}}</option>
                                    @endforeach
                                </select> <span class="input-group-btn">
                      <button type="submit" class="btn waves-effect waves-light btn-primary">View Now</button>
                      </span> </div>
                            </form>
                        </div>
                                ...
                                @foreach($data_list as $data)
                                    <tr>
                                        ...
                                        <td>{{$data->house}}, {{$data->area}}, {{$data->zone}}, {{$data->city}},{{$data->post_code}}<br> {{$data->state}}, {{$data->country}}</td>
                                            <td><a href="/edit-employee/{{$data->employee_id}}"><button class="btn btn-danger">Modify</button></a></td>
                                    </tr>
                                @endforeach
                                </tbody>
                            </table>
